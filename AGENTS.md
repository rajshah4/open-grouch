...
- format: `make format`
- run the Textual TUI (interactive; prefer running inside tmux so you can detach with `Ctrl+b d`): `make run` (or `uv run grouch`)
- run the Textual TUI (automation-friendly; use for agent-driven runs): `uv run grouch --exit-without-confirmation` (quit with `Ctrl+Q`; `Ctrl+C` does not work once the TUI is running)

- run the browser-served web app (Textual `textual-serve`): `grouch web`
- run the Docker-based OpenHands GUI server: `grouch serve`
- run the ACP entrypoint: `uv run grouch-acp`
...
- Use `grouch:` type: `grouch: add Oscar welcome messages`
...
**2. Feature/Fix PRs**
- Use `feat:` or `fix:` types (these trigger releases!)
- For actual new functionality or bug fixes

**3. Upstream Sync PRs**
- Use `sync:` type: `sync: merge upstream OpenHands-CLI changes`
